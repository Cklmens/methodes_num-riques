...
%%- ==> COMPLETE the definition of u0.m, ul.m, ur.m
global ul ur u0

payoff=1;
if payoff==1
  u0= @(s) max(K-s,0);	%- (American) Initial values (payoff function) - typically u0=@(s) max(K-s,0)
  ul= @(t) K-Smin;	%- ul= left  value, at Smin, American
  ur= @(t) 0.0;		%- ur= right value, at Smax
else
  u0= @(s) K/2*(s>=K/2 & s<=K); %- (American) Initial values (payoff function) - barrier like
  ul= @(t) 0.0;		%- ul= left  value, at Smin, American
  ur= @(t) 0.0;		%- ur= right value, at Smax
end
...
for n=0:N-1
  ...
  case 'EE-AMER'
    % COMPLETE
    % EUROPEAN SCHEME :  P = max(u0(s), (Id - dt*A)*P - dt*q(t));
    P = max(u0(s), (Id - dt*A)*P - dt*q(t));
  ...
  case 'EI-AMER-UL';
    ...
    %- pb: min(Bx-b,x-g)=0, b=Pold-dt*q(t1), g=u0(s);
    % COMPLETE:
    t1=t+dt;
    Pold=P-dt*q(t1);
    c=montee(U,P-dt*q(t1));
    P=descente_p(L,c,u0(s));

    %- Verification:
    err=norm(min(B*P-Pold,P-u0(s)));
    fprintf('Verification: err=%10.5f\n',err);

  case 'EI-AMER-NEWTON';
    if (n==0); B=Id+dt*A; end;
    %- pb: min(Bx-b,x-g)=0, b=P, g=u0(s);
    % COMPLETE 
    Pold=P;
    b=P; x0=P; g=u0(s); eps=1e-10; kmax=50;
  ...
  case 'EI-AMER-SPLITTING'
    % COMPLETE
    t1=t+dt; 
    % EUROPEAN SCHEME: P = (Id + dt*A)\(P-dt*q(t1))
    P = max(u0(s), (Id + dt*A)\(P-dt*q(t1)));
