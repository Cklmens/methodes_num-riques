...
for i=1:M+1
 ...
 while((norm(alpha-z)>e))
         z=alpha; % pour changer le alpha d'une iteration à une autre
         for j=1:N
             f(j)=max(x(j)-strike,0); % obstacle
             UU(j,i+1)=(1-alpha(j))*UU(j,i+1)+alpha(j)*f(j);
             if (B(j,:)*UU(:,i+1)- UU(j,i))<=(UU(j,i+1)-f(j))
                 alpha(j)=0;
             else
                 alpha(j)=1;
             end
            UU(j,i+1)=(1-alpha(j))*UU(j,i+1)+alpha(j)*f(j);% solution 
         end
